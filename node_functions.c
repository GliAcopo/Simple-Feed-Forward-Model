...
Output calculate_output(const Model* used_model, Prompt prompt) {
    ...
    for (size_t j = 0; j < first_layer_size; j++) {
        ...
        output.layer_inputs_and_outputs[0][j + first_layer_size] =  used_model->model_layers[0].layer_array_of_nodes[j].activation(prompt.data[j]);     // since these are two arrays stacked on top of each other, this is now the standard way of accessing the output array
    }
