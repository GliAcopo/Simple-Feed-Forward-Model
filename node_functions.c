 ...
 * @param lenght(size_t): The positive integer number, how many elements are in the prompt array
 * @param inputs_to_layers(double*): a pointer to an array of pointers to arrays (basically pointer to matrix) containing the inputs given to each layer
 * @param outputs_of_layers(double*): a pointer to an array of pointers to arrays (basically pointer to matrix) containing the outputs generated by each layer
 */
...
Output empty_output(){
    ...
    empty_output.inputs_to_layers = NULL;
    empty_output.outputs_of_layers = NULL;
    return(empty_output);
}
...
Output calculate_output(const Model* used_model, Prompt prompt) {
    ...
    /** After the first check we create the dynamically allocated arrays of the Output for the inputs_to_layers and outputs_of_layers.
     * */

    output.inputs_to_layers = 
